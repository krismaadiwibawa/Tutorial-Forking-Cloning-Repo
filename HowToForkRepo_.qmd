...
By creating a fork we are creating a clone of someone's repo in our own GitHub.
A fork is *static*.
It only mirrors the state of the source repo at the time we create the fork.

Let's say we want to create a fork of this repo: <https://github.com/linushof/BayesIntro25>
...
-   Click the button 'Fork' and 'Create a new fork'.
    ...
    ![](./Material/ForkButton.png){fig-align="center"}

-   We can name the fork as we want.
    It will not affect the source repo because it is only a copy of the source repo.
...
It is easier to interact with the files inside the fork through our local file system in our laptop or PC.
...
-   Then, `git merge upstream/main` to merge the current state of the source repo with our local folder's 'main' branch
